...
```
y  4~10   오두막과 널다리 (높이 11로 고정)
y     3   덱
y  0~ 2   물속 말뚝 — 여기는 말뚝 말고 아무것도 쓰지 않는다
y −1~−21  수직통로 (여기는 공기를 일부러 쓴다: 물을 밀어내야 한다)
y −22~    진흙 벽돌 양조장 미로 + 보스 사슬 셋 + 늪의 어머니 (21×14×21)
```
...
마을 조각은 전부 **높이 11**이고 문이 y 4~7, 직소가 y=3이다. `verify()`가 높이와 "덱 아래에
