...
0.3.0. 타이가 셋(타이가·원시 가문비·원시 소나무)에만, spacing 40 / separation 14. 밑변 39×39, 높이 56. **7층, 층마다 7×7 방 5×5 = 25칸.** 전부 `tools/generate_tower.py`가 만든다.

```
shell (39×56×39) ─안쪽→ core (35×49×35) ─안쪽→ 나머지 전부

                   5~7층   성소(21³)와 그 둘레 회랑
                   2~4층   도서관(21³)과 그 둘레 회랑
                   1층     입구, 연금실, 방들
```

**`core`는 층마다가 아니라 탑 전체에 하나짜리 상자다.** 그래야 21칸 높이의 방이 가운데 서 있고 미로가 그 둘레로 흐른다. 미로는 core의 자손이라 바깥벽에 닿지 못한다 (§10).
...
서쪽 가운데 칸에 층마다 하나씩, 아래 층이 위 층을 놓는다. 사다리가 1층 바닥부터 7층까지 통한다.

```
core → well_1 → well_2 → … → well_7
         │         │
         │         └─동→ 도서관 (2~4층)
         └─동→ 연금실        well_5 ─동→ 성소 (5~7층)
```

전부 원소가 하나뿐인 풀이라 **반드시, 정확히 한 번** 놓이고 **항상 계단실에서 바로 들어간다.** 중요한 것은 미로에 맡기지 않는다.

### 방

| 방 | 크기 | 내용 |
|---|---|---|
| `library` | 21×21×21 | 3층 높이 아트리움. 벽면 전체 책장, 회랑 둘(사다리로 오른다), 바닥 단상에 마법 부여대 + **유효 책장 15**. 상자 둘 |
| `sanctum` | 21×21×21 | 5~7층. 단상의 대마법사 + 사분면 경비 4, **불 붙이지 않은 흑요석 포탈 틀**과 화염과 강철 상자 |
| `alchemy` | 7×7×7 | 1층, 계단실 동쪽. 양조기·가마솥·상자 |
| `study` `vex_cage` `lab` | 7×7×7 | 미로. 벡스 우리는 **바닥에 선 철창 상자**다 — 성에서는 허공에 뜬 철창 한 장이었다 |

창문은 벽 두 켜를 **관통해** 뚫고 양쪽에 유리판을 넣는다. 성에서는 한 켜에만 넣어 벽에 박힌 유리처럼 보였다.
...
`docs/concepts.md` §9의 순서. CurseForge 프로젝트 1703811에 0.1.0·0.1.1·0.2.0을 배포했다. 0.1.1이 감옥 마무리, 0.2.0이 대피라미드, 0.3.0이 마법사의 탑이다. 다음은 **마녀의 늪**(양조를 네더 전에 연다).
