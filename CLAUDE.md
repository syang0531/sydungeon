...
| `sand_den` `trap_tnt` `trap_sand` | 7×7×7 | 모래 함정 (아래) |
| `cap` | 1×7×7 | 마개 |

### 모래 함정

레드스톤 없이, 바닐라 규칙만으로 작동한다.

- **`trap_tnt`** — 전리품 옆 돌 압력판이 **TNT 바로 위**에 놓여 있다. 압력판은 자기 아래 블록에 신호를 주고, TNT는 그거면 터진다. 사막 사원의 그 함정이다
- **`trap_sand`** — 통로 위에 떠 있는 모래와 그 아래 철사 트랩. 구조물은 블록 갱신 없이 블록을 놓기 때문에 모래가 공중에 떠 있고, 철사를 밟으면 상태가 바뀌면서 **위 블록에 갱신이 가서** 천장이 무너진다
- **`sand_den`** — 모래가 되찾아간 칸. 통로는 열려 있지만 나머지는 모래로 막혀 있고 알코브에 허스크 스포너가 있다

### 보상

파라오(허스크, 체력 80, 금 장비, 날카로움 III·화염 II 금검) + 경비 웨이브. 클리어하면 **흑요석 14 + 화염과 강철 확정** — 네더 포탈과 마법 부여대다 (`docs/concepts.md` §4.2). 그 위에 금·경험치·다이아·30레벨 책·사구 장식 템플릿. 지하실 상자에는 마법 황금사과가 확정으로 하나 있다.
