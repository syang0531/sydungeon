...
**목적: 파밍과 채굴 없이, 탐험만으로 엔드 콘텐츠(엔더 드래곤·풀 마법 부여·네더라이트)에 닿는다.** 바이옴마다 던전 하나, 던전마다 확정 시그니처 보상 하나. 바닐라가 파밍·채굴로 주던 것을 던전 보상으로 옮긴다. 전체 설계는 `docs/concepts.md`, 던전별 레벨 설계(조각 단면도와 조립 원리)는 `docs/levels/`다. 지하감옥·대피라미드·마법사의 탑·마녀의 늪을 만들었다. 보상 테이블에 다른 모드의 아이템은 넣지 않는다.
...
- **지붕이 없다.** 8층 천장이 탑의 끝이다. 기능은 없고 모양만 있으면 된다
- **테두리가 밋밋하다.** `ring_floor`(29×7×29)는 코드가 민 평평한 벽이다. 구조물 블록 한계
  안이라 손으로 지어 `tools/handmade/tower/`에 넣으면 생성기가 그걸 쌓는다
- **조각 종류가 적다.** 보상 방이 `dead_end` 하나뿐이라 전부 같이 생겼다. 같은 문 배치의
  조각을 여러 개 만들어 풀에 넣으면 그만큼 다양해진다 — **배선은 안 건드려도 된다**

그다음은 `docs/concepts.md` §9의 순서. CurseForge 프로젝트 1703811에 0.1.0·0.1.1·0.2.0을 배포했다. 0.1.1이 감옥 마무리, 0.2.0이 대피라미드다. 마법사의 탑은 세 번 지었다 — 0.3.0(직육면체), 0.4.0(가는 탑), **0.5.0(사람이 지은 둥근 탑)**. 0.6.0이 **마녀의 늪**이다. 그다음은 `docs/concepts.md` §9의 순서.
