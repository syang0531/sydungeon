...
> 이 문서는 `python tools/gen_level_doc.py lodge`가 만든다. 조각을 고치면 다시 돌린다. 그림과 표는 게임이 읽는 파일(`structure/lodge/*.nbt`, `worldgen/**`)에서 그대로 읽어 온 것이라 모드와 어긋날 수 없다. 설명 문장만 사람이 쓴다 (`tools/gen_level_doc.py`의 `INTRO`·`NOTES`).

## 1. 한눈에

**중소규모 던전**이다 (§2.8). 판자 오두막. 서쪽에 문, 안에 작업대와 상자, 바닥 가운데에 구멍. 그 아래 21칸을 내려가면 짧은 미로와 상자 방이 있다.

보스도 시그니처도 없고, 한 판 십 분이다. 여덟 껍질이 같은 기계에서 나오며 이것은 그중 하나다.

바이옴: dark_forest · pale_garden

| 항목 | 값 |
|---|---|
| 생성 단계 | `underground_structures` |
| 바이옴 | `#sydungeon:has_structure/lodge` |
...
|---|---|---|
| `head` | (0, 0, 0) | 시작 풀 |
| `shaft` | (0, -21, 0) | head의 아래 lodge_down 직소 |
| `hub` | (0, -28, 0) | shaft의 아래 lodge_cell 직소 |
| `store` | (0, -28, -7) | hub의 북 lodge_cell 직소 |

![뼈대](img/lodge/_assembly.svg)

![조각 지도](img/lodge/_assembly_pieces.svg)
...
시작 조각. 판자 오두막. 서쪽에 문, 안에 작업대와 상자, 바닥 가운데에 구멍. 기초는 없다 — 바닥 켜가 곧 지형의 맨 윗 블록이다 (§30).

**어느 풀에 있나** — `start` (가중치 1)

| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (4, 0, 3) | ◇ 아래 | `lodge_down` | `lodge_down` | `lodge/down` |

**뚫린 면** — 서: z 0–6, y 1–7 (23칸) / 동: z 0–6, y 6–7 (14칸) / 북: x 0–6, y 6–7 (14칸) / 남: x 0–6, y 6–7 (14칸) / 위: x 0–6, z 0–6 (49칸)

**블록** — 짙은 참나무 판자 71, 짙은 참나무 반 블록 49, 포드졸 40, 짙은 참나무 원목 16, 조약돌 7, 상자 1, 사다리 1, 직소 1

![head](img/lodge/head.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (4, 0, 3) | ◇ 아래 | `lodge_cell` | `lodge_cell` | `lodge/first` |
| (4, 20, 3) | ◆ 위 | `lodge_down` | `lodge_down` | `—` |

**블록** — 돌 806, 안산암 200, 사다리 21, 직소 2

![shaft](img/lodge/shaft.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `lodge_cell` | `lodge_cell` | `lodge/store` |
| (0, 0, 3) | ◀ 서 | `lodge_cell` | `lodge_cell` | `lodge/cells` |
| (3, 0, 6) | ▼ 남 | `lodge_cell` | `lodge_cell` | `lodge/cells` |
| (4, 6, 3) | ◆ 위 | `lodge_cell` | `lodge_cell` | `—` |

**뚫린 면** — 서: z 2–4, y 1–4 (12칸) / 북: x 2–4, y 1–4 (12칸) / 남: x 2–4, y 1–4 (12칸)

**블록** — 돌 116, 조약돌 46, 안산암 15, 사다리 6, 짙은 참나무 판자 5, 직소 4, 랜턴 1

![hub](img/lodge/hub.svg)
...
**블록** — 돌 136, 조약돌 48, 안산암 21, 짙은 참나무 판자 4, 짙은 참나무 울타리 1, 랜턴 1, 직소 1, 상자 1

![store](img/lodge/store.svg)
...
|---|---|---|---|---|
| (0, 0, 3) | ◀ 서 | `lodge_cell` | `lodge_cell` | `lodge/cells` |
| (6, 0, 3) | ▶ 동 | `lodge_cell` | `lodge_cell` | `lodge/cells` |

**뚫린 면** — 서: z 2–4, y 1–4 (12칸) / 동: z 2–4, y 1–4 (12칸)

**블록** — 돌 127, 조약돌 47, 안산암 18, 직소 2, 짙은 참나무 판자 2

![passage](img/lodge/passage.svg)
...
**블록** — 돌 127, 조약돌 47, 안산암 18, 직소 2, 짙은 참나무 판자 1, 랜턴 1

![corner](img/lodge/corner.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `lodge_cell` | `lodge_cell` | `lodge/cells` |
| (0, 0, 3) | ◀ 서 | `lodge_cell` | `lodge_cell` | `lodge/cells` |
| (6, 0, 3) | ▶ 동 | `lodge_cell` | `lodge_cell` | `lodge/cells` |
| (3, 0, 6) | ▼ 남 | `lodge_cell` | `lodge_cell` | `lodge/cells` |
...
**블록** — 돌 109, 조약돌 45, 안산암 12, 짙은 참나무 판자 12, 직소 4

![cross](img/lodge/cross.svg)
...
**블록** — 돌 127, 조약돌 38, 안산암 18, 짙은 참나무 판자 9, 직소 2, 거미줄 1, 몬스터 스포너 1

![den](img/lodge/den.svg)
...
**블록** — 돌 136, 조약돌 48, 안산암 21, 포드졸 4, 직소 1, 상자 1, 랜턴 1

![nook](img/lodge/nook.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (0, 0, 3) | ◀ 서 | `lodge_cell` | `lodge_cell` | `lodge/caps` |

**블록** — 돌 35, 안산암 13, 직소 1

![cap](img/lodge/cap.svg)
