...
> 이 문서는 `python tools/gen_level_doc.py canopy`가 만든다. 조각을 고치면 다시 돌린다. 그림과 표는 게임이 읽는 파일(`structure/canopy/*.nbt`, `worldgen/**`)에서 그대로 읽어 온 것이라 모드와 어긋날 수 없다. 설명 문장만 사람이 쓴다 (`tools/gen_level_doc.py`의 `INTRO`·`NOTES`).

## 1. 한눈에

**중소규모 던전**이다 (§2.8). 낮은 둔덕. 북쪽이 두 칸 높이로 뚫려 있고 그 안이 구멍이다. 그 아래 21칸을 내려가면 짧은 미로와 상자 방이 있다.

보스도 시그니처도 없고, 한 판 십 분이다. 여덟 껍질이 같은 기계에서 나오며 이것은 그중 하나다.

바이옴: jungle · sparse_jungle · bamboo_jungle

| 항목 | 값 |
|---|---|
| 생성 단계 | `underground_structures` |
| 바이옴 | `#sydungeon:has_structure/canopy` |
...
|---|---|---|
| `head` | (0, 0, 0) | 시작 풀 |
| `shaft` | (0, -21, 0) | head의 아래 canopy_down 직소 |
| `hub` | (0, -28, 0) | shaft의 아래 canopy_cell 직소 |
| `store` | (0, -28, -7) | hub의 북 canopy_cell 직소 |

![뼈대](img/canopy/_assembly.svg)

![조각 지도](img/canopy/_assembly_pieces.svg)
...
시작 조각. 낮은 둔덕. 북쪽이 두 칸 높이로 뚫려 있고 그 안이 구멍이다. 기초는 없다 — 바닥 켜가 곧 지형의 맨 윗 블록이다 (§30).

**어느 풀에 있나** — `start` (가중치 1)

| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (4, 0, 3) | ◇ 아래 | `canopy_down` | `canopy_down` | `canopy/down` |

**뚫린 면** — 서: z 0–6, y 2–7 (42칸) / 동: z 0–6, y 2–7 (42칸) / 북: x 0–6, y 1–7 (45칸) / 남: x 0–6, y 2–7 (42칸) / 위: x 0–6, z 0–6 (49칸)

**블록** — 이끼 낀 조약돌 68, 포드졸 40, 이끼 낀 돌벽돌 2, 사다리 1, 직소 1

![head](img/canopy/head.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (4, 0, 3) | ◇ 아래 | `canopy_cell` | `canopy_cell` | `canopy/first` |
| (4, 20, 3) | ◆ 위 | `canopy_down` | `canopy_down` | `—` |

**블록** — 돌 806, 안산암 200, 사다리 21, 직소 2

![shaft](img/canopy/shaft.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `canopy_cell` | `canopy_cell` | `canopy/store` |
| (0, 0, 3) | ◀ 서 | `canopy_cell` | `canopy_cell` | `canopy/cells` |
| (3, 0, 6) | ▼ 남 | `canopy_cell` | `canopy_cell` | `canopy/cells` |
| (4, 6, 3) | ◆ 위 | `canopy_cell` | `canopy_cell` | `—` |

**뚫린 면** — 서: z 2–4, y 1–4 (12칸) / 북: x 2–4, y 1–4 (12칸) / 남: x 2–4, y 1–4 (12칸)

**블록** — 돌 116, 이끼 낀 조약돌 46, 안산암 15, 사다리 6, 이끼 낀 돌벽돌 5, 직소 4, 랜턴 1

![hub](img/canopy/hub.svg)
...
**블록** — 돌 136, 이끼 낀 조약돌 48, 안산암 21, 이끼 낀 돌벽돌 4, 정글 울타리 1, 랜턴 1, 직소 1, 상자 1

![store](img/canopy/store.svg)
...
|---|---|---|---|---|
| (0, 0, 3) | ◀ 서 | `canopy_cell` | `canopy_cell` | `canopy/cells` |
| (6, 0, 3) | ▶ 동 | `canopy_cell` | `canopy_cell` | `canopy/cells` |

**뚫린 면** — 서: z 2–4, y 1–4 (12칸) / 동: z 2–4, y 1–4 (12칸)

**블록** — 돌 127, 이끼 낀 조약돌 47, 안산암 18, 직소 2, 이끼 낀 돌벽돌 2

![passage](img/canopy/passage.svg)
...
**블록** — 돌 127, 이끼 낀 조약돌 47, 안산암 18, 직소 2, 이끼 낀 돌벽돌 1, 랜턴 1

![corner](img/canopy/corner.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `canopy_cell` | `canopy_cell` | `canopy/cells` |
| (0, 0, 3) | ◀ 서 | `canopy_cell` | `canopy_cell` | `canopy/cells` |
| (6, 0, 3) | ▶ 동 | `canopy_cell` | `canopy_cell` | `canopy/cells` |
| (3, 0, 6) | ▼ 남 | `canopy_cell` | `canopy_cell` | `canopy/cells` |
...
**블록** — 돌 109, 이끼 낀 조약돌 45, 안산암 12, 이끼 낀 돌벽돌 12, 직소 4

![cross](img/canopy/cross.svg)
...
**블록** — 돌 127, 이끼 낀 조약돌 38, 안산암 18, 이끼 낀 돌벽돌 9, 직소 2, 거미줄 1, 몬스터 스포너 1

![den](img/canopy/den.svg)
...
**블록** — 돌 136, 이끼 낀 조약돌 48, 안산암 21, 포드졸 4, 직소 1, 상자 1, 랜턴 1

![nook](img/canopy/nook.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (0, 0, 3) | ◀ 서 | `canopy_cell` | `canopy_cell` | `canopy/caps` |

**블록** — 돌 35, 안산암 13, 직소 1

![cap](img/canopy/cap.svg)
