...
> 이 문서는 `python tools/gen_level_doc.py drip`가 만든다. 조각을 고치면 다시 돌린다. 그림과 표는 게임이 읽는 파일(`structure/drip/*.nbt`, `worldgen/**`)에서 그대로 읽어 온 것이라 모드와 어긋날 수 없다. 설명 문장만 사람이 쓴다 (`tools/gen_level_doc.py`의 `INTRO`·`NOTES`).

## 1. 한눈에

**중소규모 던전**이고, **지표 입구가 없다** (§2.8). **지상에 아무것도 없다.** 바위 속에 놓인 방 하나가 시작이고, 거기서 미로가 자란다.

`project_start_to_heightmap`을 쓰지 않고 높이를 직접 준다 — y -48에서 0 사이 어딘가의 바위
...
| 바이옴 | `#sydungeon:has_structure/drip` |
...
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `drip_cell` | `drip_cell` | `drip/store` |
| (0, 0, 3) | ◀ 서 | `drip_cell` | `drip_cell` | `drip/cells` |
| (6, 0, 3) | ▶ 동 | `drip_cell` | `drip_cell` | `drip/cells` |
| (3, 0, 6) | ▼ 남 | `drip_cell` | `drip_cell` | `drip/cells` |

**뚫린 면** — 서: z 2–4, y 1–4 (12칸) / 동: z 2–4, y 1–4 (12칸) / 북: x 2–4, y 1–4 (12칸) / 남: x 2–4, y 1–4 (12칸)

**블록** — 돌 109, 조약돌 45, 점적석 12, 직소 4, 랜턴 1, 상자 1

![hub](img/drip/hub.svg)
...
**블록** — 돌 136, 조약돌 48, 점적석 21, 돌벽돌 4, 참나무 울타리 1, 랜턴 1, 직소 1, 상자 1

![store](img/drip/store.svg)
...
**블록** — 돌 127, 조약돌 47, 점적석 18, 직소 2, 돌벽돌 2

![passage](img/drip/passage.svg)
...
**블록** — 돌 127, 조약돌 47, 점적석 18, 직소 2, 돌벽돌 1, 랜턴 1

![corner](img/drip/corner.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `drip_cell` | `drip_cell` | `drip/cells` |
| (0, 0, 3) | ◀ 서 | `drip_cell` | `drip_cell` | `drip/cells` |
| (6, 0, 3) | ▶ 동 | `drip_cell` | `drip_cell` | `drip/cells` |
| (3, 0, 6) | ▼ 남 | `drip_cell` | `drip_cell` | `drip/cells` |
...
**블록** — 돌 109, 조약돌 45, 점적석 12, 돌벽돌 12, 직소 4

![cross](img/drip/cross.svg)
...
**블록** — 돌 127, 조약돌 38, 점적석 18, 돌벽돌 9, 직소 2, 거미줄 1, 몬스터 스포너 1

![den](img/drip/den.svg)
...
**블록** — 돌 136, 조약돌 48, 점적석 25, 직소 1, 상자 1, 랜턴 1

![nook](img/drip/nook.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (0, 0, 3) | ◀ 서 | `drip_cell` | `drip_cell` | `drip/caps` |

**블록** — 돌 35, 점적석 13, 직소 1

![cap](img/drip/cap.svg)
