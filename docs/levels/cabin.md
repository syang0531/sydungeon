...
> 이 문서는 `python tools/gen_level_doc.py cabin`가 만든다. 조각을 고치면 다시 돌린다. 그림과 표는 게임이 읽는 파일(`structure/cabin/*.nbt`, `worldgen/**`)에서 그대로 읽어 온 것이라 모드와 어긋날 수 없다. 설명 문장만 사람이 쓴다 (`tools/gen_level_doc.py`의 `INTRO`·`NOTES`).
...
바이옴: taiga · old_growth_pine_taiga · old_growth_spruce_taiga

| 항목 | 값 |
|---|---|
| 생성 단계 | `underground_structures` |
| 바이옴 | `#sydungeon:has_structure/cabin` |
...
|---|---|---|
| `head` | (0, 0, 0) | 시작 풀 |
| `shaft` | (0, -21, 0) | head의 아래 cabin_down 직소 |
| `hub` | (0, -28, 0) | shaft의 아래 cabin_cell 직소 |
| `store` | (0, -28, -7) | hub의 북 cabin_cell 직소 |

![뼈대](img/cabin/_assembly.svg)

![조각 지도](img/cabin/_assembly_pieces.svg)
...
|---|---|---|---|---|
| (4, 0, 3) | ◇ 아래 | `cabin_down` | `cabin_down` | `cabin/down` |

**뚫린 면** — 서: z 0–6, y 1–7 (23칸) / 동: z 0–6, y 6–7 (14칸) / 북: x 0–6, y 6–7 (14칸) / 남: x 0–6, y 6–7 (14칸) / 위: x 0–6, z 0–6 (49칸)

**블록** — 가문비 판자 71, 가문비 반 블록 49, 거친 흙 40, 가문비 원목 16, 조약돌 7, 상자 1, 사다리 1, 직소 1

![head](img/cabin/head.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (4, 0, 3) | ◇ 아래 | `cabin_cell` | `cabin_cell` | `cabin/first` |
| (4, 20, 3) | ◆ 위 | `cabin_down` | `cabin_down` | `—` |

**블록** — 돌 806, 화강암 200, 사다리 21, 직소 2

![shaft](img/cabin/shaft.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `cabin_cell` | `cabin_cell` | `cabin/store` |
| (0, 0, 3) | ◀ 서 | `cabin_cell` | `cabin_cell` | `cabin/cells` |
| (3, 0, 6) | ▼ 남 | `cabin_cell` | `cabin_cell` | `cabin/cells` |
| (4, 6, 3) | ◆ 위 | `cabin_cell` | `cabin_cell` | `—` |

**뚫린 면** — 서: z 2–4, y 1–4 (12칸) / 북: x 2–4, y 1–4 (12칸) / 남: x 2–4, y 1–4 (12칸)

**블록** — 돌 116, 조약돌 46, 화강암 15, 사다리 6, 가문비 판자 5, 직소 4, 랜턴 1

![hub](img/cabin/hub.svg)
...
**블록** — 돌 136, 조약돌 48, 화강암 21, 가문비 판자 4, 가문비나무 울타리 1, 랜턴 1, 직소 1, 상자 1

![store](img/cabin/store.svg)
...
|---|---|---|---|---|
| (0, 0, 3) | ◀ 서 | `cabin_cell` | `cabin_cell` | `cabin/cells` |
| (6, 0, 3) | ▶ 동 | `cabin_cell` | `cabin_cell` | `cabin/cells` |

**뚫린 면** — 서: z 2–4, y 1–4 (12칸) / 동: z 2–4, y 1–4 (12칸)

**블록** — 돌 127, 조약돌 47, 화강암 18, 직소 2, 가문비 판자 2

![passage](img/cabin/passage.svg)
...
**블록** — 돌 127, 조약돌 47, 화강암 18, 직소 2, 가문비 판자 1, 랜턴 1

![corner](img/cabin/corner.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (3, 0, 0) | ▲ 북 | `cabin_cell` | `cabin_cell` | `cabin/cells` |
| (0, 0, 3) | ◀ 서 | `cabin_cell` | `cabin_cell` | `cabin/cells` |
| (6, 0, 3) | ▶ 동 | `cabin_cell` | `cabin_cell` | `cabin/cells` |
| (3, 0, 6) | ▼ 남 | `cabin_cell` | `cabin_cell` | `cabin/cells` |
...
**블록** — 돌 109, 조약돌 45, 화강암 12, 가문비 판자 12, 직소 4

![cross](img/cabin/cross.svg)
...
**블록** — 돌 127, 조약돌 38, 화강암 18, 가문비 판자 9, 직소 2, 거미줄 1, 몬스터 스포너 1

![den](img/cabin/den.svg)
...
**블록** — 돌 136, 조약돌 48, 화강암 21, 거친 흙 4, 직소 1, 상자 1, 랜턴 1

![nook](img/cabin/nook.svg)
...
| 직소 위치 | 향 | name | target | pool |
|---|---|---|---|---|
| (0, 0, 3) | ◀ 서 | `cabin_cell` | `cabin_cell` | `cabin/caps` |

**블록** — 돌 35, 화강암 13, 직소 1

![cap](img/cabin/cap.svg)
