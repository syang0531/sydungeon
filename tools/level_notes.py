...
**한 판:** 지표 정자 → 사다리 **42칸 이상** → 허브 → 미로(조각 300~1000개, 5~8층) →
어딘가의 보스방. 횃불과 침대는 상자에서 나온다 — 그게 이 던전이 처음으로 하는 약속이다.
""",

    'skeleton': """
정자와 그 밑의 수직통로 둘. **여기까지는 매번 똑같다** — `shaft_first`가 `shaft_1`을,
`shaft_1`이 `shaft_2`를 부르고, 그 아래에서야 `shafts` 풀(수직통로 3 : 허브 2)이 시작된다.
그래서 허브는 문에서 **최소 42칸 아래**다. 미로는 허브의 세 문에서 자라는 것이라 이 그림에 없다.
""",

    'floors': [('지표', 1), ('수직통로', -10), ('허브', -42)],
...
- **`stair`의 두 문이 같은 서쪽 면에 있다.** 층을 옮겼다는 감각이 약하다 — 계단을 내려와도
  같은 방향을 보고 있다. (방향 자체는 이제 한쪽뿐이다: 아래 문은 이름이 `stair_foot`이라
  미로가 붙을 수 없고, 계단은 위에서만 들어온다.)
...
NOTES['dungeon/stair'] = """
7×14×7 — 셀 두 칸을 쓰면서 한 층(7칸)을 내려간다. 층 간격이 정확히 7이라 아래층도 같은
격자에 맞는다. 가중치 5 — 던전이 5~8층이 되는 건 이 조각 하나 때문이다.

**한 방향으로만 흐른다.** 두 직소가 둘 다 서쪽 면인데, 위(y=7)만 이름이 `sydungeon:door`고
아래(y=0)는 `sydungeon:stair_foot`이다. 미로의 통로는 `door`를 찾으므로 **아래 문에는 아무도
붙을 수 없고**, 계단은 위에서만 들어온다. 둘 다 `door`였을 때는 절반이 거꾸로 붙어 미로가
내려간 만큼 올라왔고, 그래서 언덕에서는 **통로가 땅을 뚫고 밖에 서 있었다** (2026-09-22).
미로가 허브보다 높아질 수 없으면 그런 일이 없다.
"""
